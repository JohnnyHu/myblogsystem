    ...
    <div class="wroks-inner clearfix">
      <div class="blog-main">
		...
		<div class="blog-title">
			<h1>
			<h1 title="{{!item['postTitle']}}">{{!item['postTitle']}}</h1>
			</h1>
		</div>
        <div class="blog-title-info">
            <div class="title-info-list clearfix">
            <a class="title-date" href="#">{{!item['postDate']}}</a> 			<!-- 写作日期 -->
            <a class="title-sort" href="#">{{!item['termName']}}</a> 			<!-- 分类名 -->
            <a class="title-clicks" href="#">{{!item['clickCounts']}}</a> 		<!-- 点击次数 -->
            <a class="title-cmt" href="#">{{!item['commentCounts']}}</a>    	<!-- 评论次数 -->
			</div>
        </div>		
          <div class="blog-content">
            <div class="blog-txt">{{!item['postContent']}}</div>   <!-- 文章内容 -->
          </div>
        </div>		
		% end
      </div>
